...
export function ChatArea({
	...
	const channelMembers = useMemo(() => {
		...
		return workspaceSessions.map((s) => s.roleName).filter((r): r is string => Boolean(r));
	...
	const insertMention = useCallback(
		(name: string) => {
			...
			const match = beforeCursor.match(/@(\w*)$/);
			if (!match) return;
			const newBefore = beforeCursor.slice(0, beforeCursor.length - match[0].length) + `@${name} `;
	...
	useEffect(() => {
		if (searchOpen) { searchInputRef.current?.focus(); setSearchText(""); setSearchMatchIdx(0); }
	}, [searchOpen]);
	...
	useEffect(() => {
		...
		feedRef.current.scrollBy({ top: er.top - cr.top - cr.height / 2 + er.height / 2, behavior: "smooth" });
	...
	const handleChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
		...
		if (slashMatch) { setSlashQuery(slashMatch[1] ?? null); setSlashIndex(0); }
	...
	const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
		...
		if (mentionQuery !== null && mentionMatches.length > 0) {
			...
			if (e.key === "Enter") { e.preventDefault(); const n = mentionMatches[mentionIndex]; if (n) insertMention(n); return; }
	...
	const handleSlashSelect = (cmd: SlashCommand) => {
		...
		setTimeout(() => {
			textareaRef.current?.focus();
			if (textareaRef.current) { textareaRef.current.selectionStart = newCursor; textareaRef.current.selectionEnd = newCursor; }
		}, 0);
	};
	...
	const firstUnreadIdx = useMemo(() => {
		...
		return messages.findIndex((m) => m.role === "assistant" && (m.timestamp ?? 0) > lastReadTimestamp);
			...
			<div className="flex-1 flex flex-col overflow-hidden bg-card">
				...
				<div className="px-5 py-3 border-b border-border flex items-center justify-between shrink-0 min-h-[56px]">
					<div className="flex items-center gap-3 min-w-0">
						...
						<div className="min-w-0">
							<h2 className="text-[15px] font-semibold text-foreground leading-tight">{channelName}</h2>
					...
					<div className="flex items-center gap-1 shrink-0">
							...
							<div className="hidden md:flex items-center -space-x-1.5 mr-2">
								{channelMembers.slice(0, 4).map((name) => (
									<div key={name} className="relative" title={name}>
										...
										<span className={cn("absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full border-2 border-card", presenceDot(name))} />
									</div>
								))}
						...
						<button
							...
							className={cn(
								...
								searchOpen ? "bg-primary/10 text-primary" : "text-muted-foreground/60 hover:text-muted-foreground hover:bg-muted",
							)}
						>
						...
						<button
							...
							className={cn(
								"w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-150",
								settingsOpen ? "bg-primary/10 text-primary" : "text-muted-foreground/60 hover:text-muted-foreground hover:bg-muted",
							)}
						>
							<svg viewBox="0 0 16 16" fill="currentColor" className="w-4 h-4">
						...
						{workspaceSessions.some((s) => s.roleName) && onConvene && (
							<button
								type="button"
								onClick={() => { onConvene(); setConvening(true); setTimeout(() => setConvening(false), 4000); }}
					...
					<div className="px-5 py-1.5 border-b border-border bg-muted/30 flex items-center gap-4 text-[11px] text-muted-foreground shrink-0">
						...
						{respondingRoles.length > 0 ? (
							<span className="flex items-center gap-1.5 text-primary">
								...
								{respondingRoles.length === 1 ? `${respondingRoles[0]} is responding` : `${respondingRoles.join(", ")} are responding`}
							</span>
						) : convening ? (
							<span className="flex items-center gap-1.5 text-primary">
								<span className="w-1.5 h-1.5 bg-primary rounded-full animate-pulse" />
						...
						{onAddAgent && (
							<button type="button" onClick={onAddAgent} className="ml-auto flex items-center gap-1 text-[11px] text-primary/70 hover:text-primary font-medium transition-colors">
								<svg viewBox="0 0 14 14" fill="currentColor" className="w-3 h-3"><path d="M7 0a.75.75 0 01.75.75v5.5h5.5a.75.75 0 010 1.5h-5.5v5.5a.75.75 0 01-1.5 0v-5.5h-5.5a.75.75 0 010-1.5h5.5V.75A.75.75 0 017 0z" /></svg>
								+ Add Agent
							</button>
						)}
					</div>
				...
				{searchOpen && (
					<div className="flex items-center gap-2 px-5 py-2 border-b border-border bg-muted/30 shrink-0">
						<svg viewBox="0 0 16 16" fill="currentColor" className="w-3.5 h-3.5 text-muted-foreground shrink-0">
							<path fillRule="evenodd" d="M11.5 7.5a4 4 0 11-8 0 4 4 0 018 0zm-.82 4.74a5.5 5.5 0 111.06-1.06l2.79 2.79a.75.75 0 11-1.06 1.06l-2.79-2.79z" clipRule="evenodd" />
						</svg>
						<input ref={searchInputRef} type="text" value={searchText} onChange={(e) => setSearchText(e.target.value)}
							...
							onKeyDown={(e) => {
								if (e.key === "Enter") { e.preventDefault(); setSearchMatchIdx((i) => e.shiftKey ? (i - 1 + searchMatchIds.length) % Math.max(searchMatchIds.length, 1) : (i + 1) % Math.max(searchMatchIds.length, 1)); }
								if (e.key === "Escape") setSearchOpen(false);
							}}
						/>
						{searchText.trim() && (
							<span className="text-[11px] text-muted-foreground shrink-0">
								{searchMatchIds.length === 0 ? "No results" : `${searchMatchIdx + 1} / ${searchMatchIds.length}`}
							</span>
						)}
						<button type="button" onClick={() => setSearchMatchIdx((i) => (i - 1 + searchMatchIds.length) % Math.max(searchMatchIds.length, 1))} className="w-6 h-6 rounded flex items-center justify-center text-muted-foreground/50 hover:text-muted-foreground hover:bg-muted transition-colors" title="Previous">
							<svg viewBox="0 0 12 12" fill="currentColor" className="w-3 h-3"><path fillRule="evenodd" d="M6 10.5a.75.75 0 01-.75-.75V3.31L3.03 5.53a.75.75 0 01-1.06-1.06l3.75-3.75a.75.75 0 011.06 0l3.75 3.75a.75.75 0 01-1.06 1.06L6.75 3.31v6.44a.75.75 0 01-.75.75z" clipRule="evenodd" /></svg>
						</button>
						<button type="button" onClick={() => setSearchMatchIdx((i) => (i + 1) % Math.max(searchMatchIds.length, 1))} className="w-6 h-6 rounded flex items-center justify-center text-muted-foreground/50 hover:text-muted-foreground hover:bg-muted transition-colors" title="Next">
							<svg viewBox="0 0 12 12" fill="currentColor" className="w-3 h-3"><path fillRule="evenodd" d="M6 1.5a.75.75 0 01.75.75v6.44l2.22-2.22a.75.75 0 111.06 1.06L6.28 11.3a.75.75 0 01-1.06 0L1.47 7.53a.75.75 0 111.06-1.06L5.25 8.69V2.25A.75.75 0 016 1.5z" clipRule="evenodd" /></svg>
						</button>
						<button type="button" onClick={() => setSearchOpen(false)} className="w-6 h-6 rounded flex items-center justify-center text-muted-foreground/50 hover:text-destructive hover:bg-destructive/10 transition-colors" title="Close">
							<svg viewBox="0 0 10 10" fill="currentColor" className="w-2.5 h-2.5"><path d="M2.22 2.22a.75.75 0 011.06 0L5 3.94l1.72-1.72a.75.75 0 111.06 1.06L6.06 5l1.72 1.72a.75.75 0 11-1.06 1.06L5 6.06l-1.72 1.72a.75.75 0 01-1.06-1.06L3.94 5 2.22 3.28a.75.75 0 010-1.06z" /></svg>
						</button>
					</div>
				)}
				...
				<div ref={feedRef} className="flex-1 overflow-y-auto px-5 py-4">
							...
							{[0.6, 0.85, 0.4, 0.7].map((w, i) => (
								<div key={i} className="flex gap-3 animate-pulse">
									...
									<div className="flex-1 space-y-2 pt-1">
										<div className="h-3 bg-muted rounded" style={{ width: `${w * 35 + 10}%` }} />
										<div className="h-3 bg-muted/60 rounded" style={{ width: `${w * 55 + 5}%` }} />
									</div>
								</div>
							))}
						</div>
					...
					<div className="max-w-3xl mx-auto space-y-4">
						{messages.length === 0 && !isLoading && (
							<div className="flex items-center justify-center min-h-[200px]">
								<div className="text-center">
									<div className="text-[15px] font-medium text-foreground">No messages yet</div>
						...
						{messages.map((msg, idx) => {
							...
							if (isUser) {
								return (
									<div key={msg.id} data-msg-id={msg.id} className="message-enter">
										{showUnreadSeparator && <UnreadSeparator />}
										<div className="flex justify-end gap-2 group/msg">
											<div className={cn("max-w-[75%] min-w-0", isCurrentMatch && "ring-2 ring-primary/40 rounded-2xl")}>
												...
												<div className={cn(
													...
													isSearchMatch && !isCurrentMatch && "ring-2 ring-primary/30"
												)}>
													{msg.content}
												</div>
							...
							if (msg.role === "system") {
								return (
									<div key={msg.id} className="flex justify-center message-enter">
										...
										<div className="flex gap-3">
											...
											<div className="flex-1 min-w-0">
												<div className="flex items-center gap-2 mb-1.5">
													<span className="text-[14px] font-semibold text-foreground">{role.name}</span>
													<span className="px-1.5 py-0.5 rounded text-[10px] font-semibold text-white leading-none" style={{ backgroundColor: role.tagColor }}>
														{role.tag}
													</span>
													{isCoordinator && (
														<span className="px-1.5 py-0.5 rounded text-[9px] font-bold text-white bg-primary leading-none">LEAD</span>
													)}
													<div className="ml-auto flex items-center gap-1.5 opacity-0 group-hover/msg:opacity-100 transition-opacity">
														...
														<span className="text-[11px] text-muted-foreground">{relativeTime(msg.timestamp)}</span>
													</div>
												</div>
												<MessageBubble message={msg} isStreaming={msg.isStreaming} hideHeader />
											</div>
										</div>
									</div>
								);
							...
							// ── Assistant (default) ──
							return (
								<div key={msg.id} data-msg-id={msg.id} className={cn("group/msg message-enter", (isSearchMatch || isCurrentMatch) && "rounded-xl", isCurrentMatch && "ring-2 ring-primary/40")}>
									{showUnreadSeparator && <UnreadSeparator />}
									<div className="flex gap-3">
										...
										<div className="flex-1 min-w-0">
											<div className="flex items-center gap-2 mb-1.5">
												<span className="text-[14px] font-semibold text-foreground">JCode</span>
												<span className="text-[11px] text-muted-foreground ml-auto opacity-0 group-hover/msg:opacity-100 transition-opacity">{relativeTime(msg.timestamp)}</span>
											</div>
											<MessageBubble message={msg} isStreaming={msg.isStreaming} hideHeader />
										</div>
									</div>
						...
						{(isProcessing || respondingRoles.length > 0) && (
							<div className="flex flex-col gap-2 pl-1">
								{(respondingRoles.length > 0 ? respondingRoles : [null]).map((role) => (
				...
				<div className="px-4 pb-3 pt-2 border-t border-border bg-card">
					<div className="max-w-3xl mx-auto relative">
						...
						{slashQuery !== null && (
							<SlashCommandPalette query={slashQuery} selectedIndex={slashIndex} onIndexChange={setSlashIndex} onSelect={handleSlashSelect} />
						)}
						...
						{mentionQuery !== null && mentionMatches.length > 0 && (
							<div className="absolute bottom-full left-0 right-0 mb-2 bg-card border border-border rounded-xl shadow-lg overflow-hidden z-50 animate-fade-in">
								...
								{mentionMatches.map((name, i) => {
									...
									return (
										<button key={name} type="button" onMouseDown={(e) => { e.preventDefault(); insertMention(name); }}
											className={cn("w-full text-left px-3 py-2 flex items-center gap-2.5 text-[13px] transition-colors", i === mentionIndex ? "bg-primary/10" : "hover:bg-muted")}>
											<div className="relative shrink-0">
												<AgentAvatar name={name} size="sm" />
												<span className={cn("absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full border-2 border-card", as?.liveProcessing ? "bg-primary animate-pulse" : "bg-emerald-500")} />
											</div>
											<div className="flex-1 min-w-0">
												<span className={cn("font-medium", i === mentionIndex ? "text-primary" : "text-foreground")}>{name}</span>
											</div>
											<span className="text-[10px] font-semibold px-1.5 py-0.5 rounded text-white shrink-0 leading-none" style={{ backgroundColor: mr.tagColor }}>
												{mr.tag}
											</span>
										</button>
									);
						...
						<div className="rounded-xl border border-border bg-card shadow-sm overflow-hidden focus-within:border-primary/50 focus-within:ring-1 focus-within:ring-primary/20 transition-all duration-150">
							...
							<div className="flex items-center justify-between px-3 py-1.5 border-t border-border">
								<div className="flex items-center gap-0.5">
									{/* Attach */}
									<button type="button" className="w-7 h-7 rounded-lg flex items-center justify-center text-muted-foreground/50 hover:text-muted-foreground hover:bg-muted transition-all duration-150" title="Attach">
										<svg viewBox="0 0 16 16" fill="currentColor" className="w-4 h-4">
											<path fillRule="evenodd" d="M12.5 3.5a2 2 0 00-2.83 0l-5.5 5.5a2.5 2.5 0 003.54 3.54l4.5-4.5a.75.75 0 011.06 1.06l-4.5 4.5a4 4 0 01-5.66-5.66l5.5-5.5a3.5 3.5 0 014.95 4.95l-5.5 5.5A2.12 2.12 0 016.5 11.5a.75.75 0 010-1.5c.54 0 1.07-.22 1.47-.61l5.5-5.5a2 2 0 000-2.83z" clipRule="evenodd" />
										</svg>
									</button>
									{/* @mention */}
									<button type="button" title="Mention agent" onClick={() => {
										const ta = textareaRef.current;
										if (!ta) return;
										const cursor = ta.selectionStart ?? text.length;
										setText(text.slice(0, cursor) + "@" + text.slice(cursor));
										setMentionQuery(""); setMentionIndex(0);
										setTimeout(() => { ta.focus(); ta.selectionStart = cursor + 1; ta.selectionEnd = cursor + 1; }, 0);
										...
										<svg viewBox="0 0 16 16" fill="currentColor" className="w-4 h-4">
											<path fillRule="evenodd" d="M8 1a7 7 0 107 7 1 1 0 00-2 0 5 5 0 11-1.5-3.53 1 1 0 001.5-1.33A7 7 0 108 1zm0 4a2 2 0 100 4 2 2 0 000-4z" clipRule="evenodd" />
										</svg>
									</button>
								</div>
								...
								<div className="flex items-center gap-2">
									{isProcessing && (
										<button type="button" onClick={onCancel} className="px-3 py-1.5 rounded-lg text-[12px] font-medium text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all duration-150">
											Cancel
										</button>
									)}
									<button
