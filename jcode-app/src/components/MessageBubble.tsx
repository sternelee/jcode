...
function classifySystem(content: string): {
	...
	if (content.includes("Queued") || content.includes("queued"))
...
function systemBadges(content: string, kind: SystemKind): string[] {
	...
	if (kind === "rewind")
		return [
			content.match(/message\s+(\d+)/)?.[1]
				? `msg ${content.match(/message\s+(\d+)/)?.[1]}`
				: "",
		].filter(Boolean);
	if (kind === "stdin")
		return [content.match(/requested by\s+(.+?)\s+\(/)?.[1] || ""].filter(
			Boolean,
		);
	if (kind === "queue") {
...
export function MessageBubble({
	...
	if (!isUser && message.roleName) {
		...
		return (
			<div
				className={cn(
					"group flex gap-3 px-3 py-2 rounded-xl transition-colors hover:bg-muted/30",
