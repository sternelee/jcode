...
function classifySystemMessage(content: string): {
	...
	if (
		...
		content.includes("interactive input")
	) {
...
function systemMetaBadges(content: string, kind: SystemMessageKind): string[] {
	if (kind === "history") {
		...
		return [messageCount ? `${messageCount} messages` : "", model || ""].filter(
			Boolean,
		);
	}
	if (kind === "compaction") {
					...
					<div className="flex items-baseline gap-2 mb-1.5">
						...
						{message.tokenUsage && (
							...
							>
								...
								{message.tokenUsage.cacheReadInput !== undefined &&
									message.tokenUsage.cacheReadInput > 0 && (
										<span
											className="text-emerald-600 dark:text-emerald-400 ml-1"
											title={`Cache read: ${message.tokenUsage.cacheReadInput}`}
										>
											cache↑{message.tokenUsage.cacheReadInput}
										</span>
									)}
								{message.tokenUsage.cacheCreationInput !== undefined &&
									message.tokenUsage.cacheCreationInput > 0 && (
										<span
											className="text-amber-600 dark:text-amber-400 ml-1"
											title={`Cache write: ${message.tokenUsage.cacheCreationInput}`}
										>
