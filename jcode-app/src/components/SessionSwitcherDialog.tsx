...
import { Search, Users, Sparkles, Circle, Loader2 } from "lucide-react";
...
function statusBadge(
	...
	if (s.includes("crash") || s.includes("error") || s.includes("fail"))
		return { label: session.status!, variant: "destructive" };
	if (session.livePhase === "chunking" || session.livePhase === "thinking")
		return { label: session.livePhase, variant: "default" };
									...
									<div className="flex items-center gap-2 min-w-0 flex-wrap">
										...
										{badge && (
											<Badge
												...
												className="h-[18px] text-[9px] leading-none px-1.5"
											>
												{badge.label}
											</Badge>
										)}
