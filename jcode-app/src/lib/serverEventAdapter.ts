...
export type DesktopSemanticEvent =
  ...
  | { type: "swarm-status"; members: import("@/types").SwarmMemberStatusSnapshot[] }
...
function historyMessageToChatMessage(message: HistoryMessage, index: number): ChatMessage {
...
function toPreviewItems(items: Extract<ServerEvent, { type: "swarm_plan" }>['items'] | Extract<ServerEvent, { type: "swarm_plan_proposal" }>['items']) {
  return [...items]
    .sort((a, b) => {
      const statusRank = (status: string, blockedBy?: string[]) => {
        const normalized = status.toLowerCase();
        if (normalized === "running" || normalized === "running_stale") return 0;
        if (normalized === "blocked" || (blockedBy?.length || 0) > 0) return 2;
        if (["queued", "ready", "pending", "todo"].includes(normalized)) return 1;
      ...
      return statusRank(a.status, a.blocked_by) - statusRank(b.status, b.blocked_by)
...
function buildSwarmPlanSummary(event: Extract<ServerEvent, { type: "swarm_plan" }>): SwarmPlanSummary {
  ...
  return {
    ...
    reason: event.reason,
    readyCount: summary?.ready_ids.length ?? event.items.filter((item) => ["queued", "ready", "pending", "todo"].includes(item.status.toLowerCase())).length,
    activeCount: summary?.active_ids.length ?? event.items.filter((item) => ["running", "running_stale"].includes(item.status.toLowerCase())).length,
    blockedCount: summary?.blocked_ids.length ?? event.items.filter((item) => item.status.toLowerCase() === "blocked").length,
    completedCount: summary?.completed_ids.length ?? event.items.filter((item) => ["completed", "done"].includes(item.status.toLowerCase())).length,
  ...
  switch (event.type) {
    ...
    case "history": {
      ...
      const restoredHistoryMarker: ChatMessage = {
        ...
        role: "system",
        content: `📚 Restored session history (${historyMessages.length} messages)` +
      ...
      if (event.total_tokens) {
        desktopEvents.push({ type: "total-tokens", tokens: event.total_tokens });
      }
