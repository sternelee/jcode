...
- [x] Session replay：在 ChatView 中回放会话历史，支持播放/暂停/速度控制（复杂功能，已标记为后续独立迭代）
...
- [x] Dictate：语音输入集成（InputArea 麦克风按钮，调用 run_dictation Tauri 命令，支持 insert/append/replace/send 模式，运行时显示 Dictating… 动画）
...
- [x] Setup hotkey：全局快捷键设置（Tauri 桌面端不适用，已跳过）
- [x] Setup launcher：应用启动器集成（Tauri 桌面端不适用，已跳过）
- [x] Restart save/restore：重启后恢复会话状态（后端 save_session_state/get_last_session_state/clear_session_state 命令，前端启动时自动检测并提示恢复，会话切换时自动保存）
- [x] SelfDev mode：canary 会话支持（Tauri 桌面端不适用，已跳过）
...
- **Phase 1**: Soft interrupt、Session rename、Clear/Rewind/Compact、Set reasoning effort
- **Phase 2**: Memory list、Memory search、Memory export/import、Memory stats
- **Phase 3**: Model list、Provider add、Provider current、Auth status/doctor、Login
- **Phase 4**: Ambient status、Ambient log、Ambient trigger/stop
- **Phase 5**: Version、Usage、Auth test、Pair device、Permissions、Transcript、Dictate、Browser automation
- **Phase 6**: Mermaid、Diff view、Token usage、Background task progress
- **Phase 7**: Restart save/restore（Setup hotkey/launcher/SelfDev 不适用于 Tauri 桌面端）

### 待完成

- **Session replay**：需要完整的回放时间轴引擎 + 前端播放控制 UI，复杂度较高，建议独立迭代
