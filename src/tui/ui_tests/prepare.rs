...
#[test]
fn test_prepare_messages_shows_live_tool_progress_in_chat_history() {
    let state = TestState {
        display_messages: vec![
            DisplayMessage::user("test it"),
            DisplayMessage::assistant("I'll run the focused tests."),
        ],
        ...
        streaming_tool_calls: vec![ToolCall {
            id: "tool-1".to_string(),
            name: "bash".to_string(),
            input: serde_json::json!({"command": "cargo test -p jcode ui_tests::prepare"}),
            intent: Some("Run focused tests".to_string()),
        }],
        ..Default::default()
    };
    ...
    let assistant_idx = rendered
        .iter()
        .position(|line| line.contains("focused tests"))
        .expect("missing assistant text");
    let tool_idx = rendered
        .iter()
        .position(|line| line.contains("⠂ bash · Run focused tests · $ cargo"))
        .expect("missing live tool progress row");

    assert!(
        assistant_idx < tool_idx,
        "live tool row should render after committed assistant text in {rendered:?}"
    );
    assert!(
        rendered.iter().any(|line| line.contains("●·· ⠂ bash")),
        "missing animated left halo in {rendered:?}"
    );
    assert!(
        rendered.iter().any(|line| line.contains("··●")),
        "missing animated right halo in {rendered:?}"
    );
}

#[test]
fn test_prepare_messages_live_tool_halo_advances_between_frames() {
    ...
    let first = TestState {
        status: ProcessingStatus::RunningTool("bash".to_string()),
        anim_elapsed: 0.0,
        streaming_tool_calls: vec![tool_call.clone()],
        ..Default::default()
    };
    let second = TestState {
        status: ProcessingStatus::RunningTool("bash".to_string()),
        anim_elapsed: 0.2,
        streaming_tool_calls: vec![tool_call],
        ..Default::default()
    };
    ...
    assert!(
        first_rendered
            .iter()
            .any(|line| line.contains("●·· ⠂ bash $ sleep 1")),
        "expected first live tool frame in {first_rendered:?}"
    );
    assert!(
        second_rendered
            .iter()
            .any(|line| line.contains("·●· ⠇ bash $ sleep 1")),
        "expected later live tool frame in {second_rendered:?}"
    );
    assert_ne!(
        first_rendered, second_rendered,
        "live tool progress should rerender as halo/spinner advances"
    );
}
